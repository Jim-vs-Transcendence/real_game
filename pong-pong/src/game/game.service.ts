...
import { Room } from 'src/game/data/playerData';
...
export class GameService {
    ...
    public test(room) {
        const idx: number = this.gameGateway.getRooms().indexOf(room);
        // 전적 추가
        // 재시작 여부 판단 로직 추가
        if (idx !== -1) {
            this.gameGateway.getRooms().splice(idx, 1);
        }
    ...
    public resetGame(room: Room): void {
        ...
        if (room.leftPlayer.leftScore >= 3 || room.leftPlayer.rightScore >= 3) {
            ...
            if (room.leftPlayer.leftScore >= 3) {
                this.gameGateway.server.to(room.leftPlayer.socketId).emit('endGame', true);
                this.gameGateway.server.to(room.rightPlayer.socketId).emit('endGame', false);
            }
            else {
                this.gameGateway.server.to(room.leftPlayer.socketId).emit('endGame', false);
                this.gameGateway.server.to(room.rightPlayer.socketId).emit('endGame', true);
            }
    ...
    public sendGameData(room: Room) {
        ...
        if (!room.isEnd) {
            ...
            this.gameGateway.server.to(room.leftPlayer.socketId).emit('ballMove', room.leftPlayer);
            this.gameGateway.server.to(room.rightPlayer.socketId).emit('ballMove', room.rightPlayer);
        }
    }
